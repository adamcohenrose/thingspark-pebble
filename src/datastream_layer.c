...
//  APP_LOG(APP_LOG_LEVEL_DEBUG, "-> datastream_layer_draw_graph");
  ...
  for (int idx = 1; idx < GRAPH_WIDTH; idx++) {
    if ((*datastream_layer)->pixel[idx-1] >= 0  && 
      (*datastream_layer)->pixel[idx] >= 0 && 
      (*datastream_layer)->pixel[idx-1] < LAYER_HEIGHT && 
      (*datastream_layer)->pixel[idx] < LAYER_HEIGHT) {
      graphics_draw_line(ctx, GPoint(idx-1, (*datastream_layer)->pixel[idx-1]), 
                 GPoint(idx, (*datastream_layer)->pixel[idx]));
    }
  }
...
//  APP_LOG(APP_LOG_LEVEL_DEBUG, "-> datastream_layer_create %u %s", (unsigned int)feed, channel);
  ...
  layer->fieldNum = fieldNum;
  if (apiKey != NULL) {
    copy_string(apiKey, layer->apiKey, sizeof(layer->apiKey));
  } else {
    layer->apiKey[0] = '\0';
  }
  ...
  text_layer_set_text_color(layer->textMax, GColorLightGray);
  ...
  text_layer_set_text_color(layer->text, GColorDarkGray);
  ...
  text_layer_set_text_color(layer->textMin, GColorLightGray);
...
//  APP_LOG(APP_LOG_LEVEL_DEBUG, "-> datastream_layer_set_icon %u", resource);
  ...
  if (layer->resource != resource) {
    ...
    if (resource != 0) {
      ...
      GBitmap* icon_bitmap = gbitmap_create_with_resource(layer->resource);
      bitmap_layer_set_bitmap(layer->icon, icon_bitmap);
      bitmap_layer_set_compositing_mode(layer->icon, GCompOpSet);
...
//  APP_LOG(APP_LOG_LEVEL_DEBUG, "-> request_datastream %u, %s", (unsigned int)layer->feed, layer->channel);
  ...
  dict_write_uint32(iter, CHANNEL_ID_KEY, layer->channelId);
  dict_write_uint32(iter, FIELD_NUM_KEY, layer->fieldNum);
  dict_write_uint8(iter, CHART_HEIGHT_KEY, LAYER_HEIGHT);
  dict_write_cstring(iter, API_KEY_KEY, layer->apiKey);
  dict_write_uint32(iter, GRAPH_WIDTH_KEY, GRAPH_WIDTH);
  dict_write_end(iter);
